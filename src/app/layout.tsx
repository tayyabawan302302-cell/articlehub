...
const display = Playfair_Display({ subsets: ["latin"], variable: "--font-display", weight: ["500", "600", "700", "900"] });
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: siteName, description, siteName, type: "website" },
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const organizationJsonLd = {
    ...
    sameAs: [settings?.facebook_url, settings?.linkedin_url, settings?.x_url, settings?.instagram_url].filter(Boolean),
  };

  // Theme Color from Site Settings actually drives the site now — validated
  // as a real hex color before being injected, since it comes from the DB
  // and lands directly in a <style> tag.
  const isValidHex = settings?.theme_color && /^#[0-9A-Fa-f]{6}$/.test(settings.theme_color);
  ...
  return (
    <html lang="en" className={`${display.variable} ${body.variable} ${mono.variable}`}>
      ...
      <body>
        ...
        <Navbar siteName={settings?.site_name ?? "ArticleHub"} navItems={navItems ?? []} />
