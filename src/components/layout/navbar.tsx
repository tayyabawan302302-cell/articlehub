...
export default async function Navbar({
  ...
  // Get the active header advertisement from Supabase
  const { data: headerAd } = await supabase
    .from("advertisements")
    .select("adsense_code")
    .eq("placement", "header")
    .eq("is_active", true)
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <>
                <Link
                  href="/login"
                  className="hidden sm:inline-block text-sm font-medium text-ink hover:text-denim-dark"
                >
                  Log in
                </Link>
      ...
      {headerAd?.adsense_code && (
        <div className="mx-auto max-w-6xl px-6 py-4">
          <AdsenseAd code={headerAd.adsense_code} />
        </div>
      )}
    </>
  );
